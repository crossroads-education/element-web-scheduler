...
export default class SchedulerData {
    constructor(initData = { events: [], resources: [], adornments: [], date: moment().format(DATE_FORMAT)}, 
                viewType = ViewTypes.Week,
                showAgenda = false, isEventPerspective = false,
                newConfig = undefined, newBehaviors = undefined,
                localeMoment = undefined) {
        const {resources, events, adornments, date} = initData;     
        ...
        if (events.length   ) this._generateEventGroups();
        
        if (this.config.recurringEventsEnabled && events.length) this._handleRecurringEvents();
    ...
    setEventGroups(eventGroups) {
        this.eventGroups = _.cloneDeep(eventGroups)
    ...
    setAdornments(adornments) {
        this.adornments = _.cloneDeep(adornments);
        this._createRenderData();
    }
    ...
    getContentTableWidth(){
        return this.config.schedulerContentWidth - this.getResourceTableWidth();
    }
    ...
    _createInitRenderData(isEventPerspective, eventGroups, resources, headers, adornments) {
    ...
    _createRenderData() {
        let initRenderData = this._createInitRenderData(this.isEventPerspective, this.eventGroups, this.resources, this.headers, this.adornments);
        this.events.forEach((item) => {
